...
public class UDPPacket {
    ...
    float[] m_susp_pos = new float[4];	// Note: All wheel arrays have the order:
    float[] m_susp_vel = new float[4];	// RL, RR, FL, FR
    float[] m_wheel_speed = new float[4];
    float   m_throttle;
    float   m_steer;
    float   m_brake;
    float   m_clutch;
    ...
    float[] m_brakes_temp = new float[4];	// brakes temperature (centigrade)
    float[] m_tyres_pressure = new float[4];	// tyres pressure PSI
    ...
    byte[]  m_tyres_temperature = new byte[4];	// tyres temperature (centigrade)
    byte[]  m_tyres_wear = new byte[4];	// tyre wear percentage
    ...
    byte[]  m_tyres_damage = new byte[4];	// tyre damage (percentage)
    ...
    byte[]  m_car_data = new byte[20];   // data for all cars on track
    ...
    float[] m_susp_acceleration = new float[4];   // NEW (v1.8) RL, RR, FL, FR
